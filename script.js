...
// WAVEFORM VISUALIZER
(function() {
  const audioEl = document.getElementById('analyser-source');
  if (!audioEl) return;
  ...
  function draw() {
    ...
    ctx.strokeStyle = getComputedStyle(document.documentElement)
                      .getPropertyValue('--brand-teal') || '#5A8785';
    ctx.beginPath();
...
;(function(){
  ...
  function draw() {
    ...
    ctx.fillRect(0,0,canvas.width,canvas.height);
    ctx.lineWidth=2;
    ctx.strokeStyle=getComputedStyle(document.documentElement).getPropertyValue('--brand-teal')||'#5A8785';
    ctx.beginPath();
    let x=0;
